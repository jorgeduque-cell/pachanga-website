...
// POST /api/events/:id/map — Upload/replace the concert seating map (admin)
router.post('/:id/map', authenticate, requireAdmin, rawBodyParser, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const id = UuidSchema.parse(req.params.id);
        const contentType = (req.headers['content-type'] || 'image/jpeg') as string;
        ...
        const rawFileName = (req.headers['x-filename'] as string) || 'map.jpg';
        const safeFileName = rawFileName.replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 100);

        const event = await eventsService.uploadMap(id, buffer, safeFileName, contentType);
        res.json(event);
    } catch (error) {
        if (error instanceof z.ZodError) {
            res.status(400).json({ error: 'ID inválido' });
